...
std::string clearHtmlContent(std::string& html) {
	...
	html.erase(std::remove_if(html.begin(), html.end(), ispunct), html.end());
	
	//фильтруем слова по размеру, раздел€€ пробелами
	...
	return boost::locale::to_lower(result);
			...
			//}
			//else
			//// –азбор нового URL (разделение на host и query)
			//// Ќапример, parseUrl(newLocation, host, query);
			//redirect_count++;
			//continue;
