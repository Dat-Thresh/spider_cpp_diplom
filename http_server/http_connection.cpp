...
void HttpConnection::createResponsePost()
{
	if (request_.target() == "/")
	{
		...
		//подключаемся к ДБ и по value|utf8value ищем наиболее релевантный результат по ссылкам
		//выдаем это в виде вектора со строками с urlами, где s встерччается чаще всего	
